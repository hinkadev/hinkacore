...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::Hinka, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (hinka.conf).");
            QMessageBox::warning(this, "Hinka Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
void BlockExplorer::setContent(const std::string& Content)
{
    ...
    QString CSS = "body {font-size:12px; color:#dddddd; bgcolor:#151515;}\n a, span { font-family: monospace; }\n span.addr {color:#dddddd; font-weight: bold;}\n\
table tr td {padding: 3px; border: 1px solid black; background-color: #151515;}\n td.d0 {font-weight: bold; color:#dddddd;}\n h2, h3 { white-space:nowrap; color:#dddddd;}\n\
a { color:#ffffff; text-decoration:none; }\n a.nav {color:#dddddd;}\n";
    QString FullContent = "<html><head><style type=\"text/css\">" + CSS + "</style></head>" + "<body bgcolor=#151515>" + Content.c_str() + "</body></html>";
