...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            if (tx.ContainsZerocoins()) //zerocoin is disabled
                continue;
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zHinka spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zHinka serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HinkaMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HinkaMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HinkaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hinka-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running HinkaMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadHINMiner(void* parg)
{
    boost::this_thread::interruption_point();
    ...
    try {
        ...
        LogPrintf("ThreadHINMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHINMiner() exception");
    }

    LogPrintf("ThreadHINMiner exiting\n");
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadHINMiner, pwallet));
}
