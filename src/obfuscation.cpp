    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Hinka and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 Hinka per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    while (it != vecObfuscationQueue.end()) {
        if ((*it).IsExpired()) {
            LogPrint("obfuscation", "CObfuscationPool::CheckTimeout() : Removing hinkared queue entry - %d\n", c);
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE) {
        ...
        while (it2 != entries.end()) {
            if ((*it2).IsExpired()) {
                LogPrint("obfuscation", "CObfuscationPool::CheckTimeout() : Removing hinkared entry - %d\n", c);
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeHinkaAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100Hinka+1 ( bit on if present )
    // bit 1 - 10Hinka+1
    // bit 2 - 1Hinka+1
    // bit 3 - .1Hinka+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        BOOST_FOREACH (CTxOut out, txVin.vout) {
			LogPrint("masternode", "IsVinAssociatedWithPubkey: amount is %f Hinka\n", out.nValue / COIN);
            if (out.nValue == mnActivateAmount) {
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("hinka-obfuscation");
