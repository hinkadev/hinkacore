...
#if defined(NDEBUG)
#error "Hinka cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are disabled at Hinka"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zHinka spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zHinka spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zHinka spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 40000 * COIN;  //premine 40,000
    } else if(nHeight >=2 && nHeight <= Params().LAST_POW_BLOCK()) { //PoW phase
        nSubsidy = 0.1 * COIN;
    } else if(nHeight >Params().LAST_POW_BLOCK() && nHeight < 5000) { //PoS phase
        nSubsidy = 0.1 * COIN;
    } else if(nHeight >= 5000 && nHeight < 20000) {
        nSubsidy = 1.3 * COIN;
    } else if(nHeight >= 20000 && nHeight < 70000) {
        nSubsidy = 1.0 * COIN;
        ...
        nSubsidy = 0.2 * COIN;
    } else if(nHeight >= 1000000 && nHeight < 10000000) {
        nSubsidy = 0.1 * COIN;
    } else if(nHeight >= 10000000) {
        int nMul = (nHeight-9000000)/350000;
        if(nMul < 32)
			nSubsidy = (0.1 / pow(2,nMul)) * COIN;
		else
			nSubsidy = 0; //down to zero
    } else {
        nSubsidy = 0.01 * COIN;  //everything else
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
		  ...
		  ret = blockValue / 100 * 90; //90% for MN
	else
		ret = blockValue / 100 * 99; //99% for MN
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Hinka
...
{
    RenameThread("hinka-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZHinkaMinted()
{
...
void RecalculateZHinkaSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zHinka supply
...
bool RecalculateHinkaSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.ContainsZerocoins())
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are disabled at Hinka"));
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zHinka with serial %s is already in the block %d\n",
    ...
    //LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHinkaSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert Hinka to zHinka
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zHinka spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zHinka serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zHinka mints and %d zHinka spends\n", __func__, nMints, nSpends);
...
bool LoadExternalBlockFile(FILE* fileIn, CDiskBlockPos* dbp)
{
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                // locate a header
                unsigned char buf[MESSAGE_STHinka_SIZE];
                blkdat.FindByte(Params().MessageStart()[0]);
                nRewind = blkdat.GetPos() + 1;
                blkdat >> FLATDATA(buf);
                if (memcmp(buf, Params().MessageStart(), MESSAGE_STHinka_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Hinka: We use certain sporks during IBD, so check to see if they are
    ...
    // getaddr message mitigates the attack.
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                // duplicate/hinkared/invalid-signature/whatever alerts
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_STHinka_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INVALID MESSAGESTHinka %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            fOk = false;
            break;
        }
