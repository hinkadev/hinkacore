...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Hinka users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Hinka " + FormatFullVersion();
...
void static ThreadHINStakeMinter()
{
    boost::this_thread::interruption_point();
    LogPrintf("ThreadHINStakeMinter started\n");
    CWallet* pwallet = pwalletMain;
    try {
        BitcoinMiner(pwallet, true);
        boost::this_thread::interruption_point();
    } catch (std::exception& e) {
        LogPrintf("ThreadHINStakeMinter() exception \n");
    } catch (...) {
        LogPrintf("ThreadHINStakeMinter() error \n");
    }
    LogPrintf("ThreadHINStakeMinter exiting,\n");
}
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Hinka Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (GetBoolArg("-staking", true))
        threadGroup.create_thread(boost::bind(&TraceThread<void (*)()>, "stakemint", &ThreadHINStakeMinter));
}
