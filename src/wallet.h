...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 Hinka at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zHinka send
enum ZerocoinSpendStatus {
    ZHinka_SPEND_OKAY = 0,                            // No error
    ZHinka_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZHinka_WALLET_LOCKED = 2,                         // Wallet was locked
    ZHinka_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZHinka_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZHinka_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZHinka_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZHinka_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZHinka_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZHinka_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZHinka_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZHinka_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZHinka_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZHinka_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZHinka_SPENT_USED_ZHinka = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void SetNull()
    {
        ...
        //Auto Combine Dust - Hinka uses autocomibe dust with 20 COINS minimum
        fCombineDust = false;
    ...
    void setZHinkaAutoBackups(bool fEnabled)
    {
...
/** Private key that includes an hinkaration date in case it never gets used. */
class CWalletKey
{
public:
    CPrivKey vchPrivKey;
