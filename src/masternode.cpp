...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!IsPingedWithin(MASTERNODE_HINRATION_SECONDS)) {
        activeState = MASTERNODE_HINRED;
        return;
    }
    ...
    if (!unitTest) {
	    ...
	    mnActivateSurvive = CMasternodeBroadcast::GetMasternodeCollateralToActivate(nHeight);
...
std::string CMasternode::GetStatus()
{
    ...
    case CMasternode::MASTERNODE_ENABLED:
        return "ENABLED";
    case CMasternode::MASTERNODE_HINRED:
        return "HINRED";
    case CMasternode::MASTERNODE_OUTPOINT_SPENT:
        return "OUTPOINT_SPENT";
    case CMasternode::MASTERNODE_REMOVE:
        return "REMOVE";
    case CMasternode::MASTERNODE_WATCHDOG_HINRED:
        return "WATCHDOG_HINRED";
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOfhinka)
{
    CTxIn txin;
    ...
    if (!fOfhinka && !masternodeSync.IsBlockchainSynced()) {
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 10000 Hinka tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 Hinka tx -> 1 confirmation
