...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        ...
        QString address = "<span style='font-family: monospace; color:red'>" + rcp.address;
        address.append("</span>");
...
void SendCoinsDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QStringList formatted)
{
    ...
    QString questionString = tr("<span style='color:white'>Are you sure you want to send?");
    ...
    questionString.append("<hr />");
    questionString.append(tr("<b>(%1 of %2 entries displayed)</b></span>").arg(displayedEntries).arg(messageEntries));
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Hinka address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
