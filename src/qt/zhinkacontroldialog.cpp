...
#include "ui_zhinkacontroldialog.h"
...
std::list<std::string> ZHinkaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHinkaControlDialog::listMints;

ZHinkaControlDialog::ZHinkaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHinkaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHinkaControlDialog::~ZHinkaControlDialog()
{
    delete ui;
}

void ZHinkaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHinkaControlDialog::updateList()
{
...
void ZHinkaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHinkaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHinka_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHinkaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHinkaControlDialog::GetSelectedMints()
{
...
void ZHinkaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
