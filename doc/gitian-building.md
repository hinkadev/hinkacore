...
*Setup instructions for a gitian build of Hinka using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Hinka
...
Clone the git repositories for hinka and gitian and then checkout the hinka version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the hinka repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work ofhinka.

Building Hinka
----------------

To build Hinka (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the hinka repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hinka/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/Hinka
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Hinka.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit hinka=${COMMIT} --url hinka=${URL} ../Hinka/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hinka=${COMMIT} --url hinka=${URL} ../Hinka/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit hinka=${COMMIT} --url hinka=${URL} ../Hinka/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hinka-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hinka-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/hinka-build.assert
```
