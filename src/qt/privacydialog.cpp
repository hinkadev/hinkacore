                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zHinka ought to be enough for anybody." - Bill Gates, 2017
    ui->zHinkapayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHinkaSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzHinka->setEnabled(false);
        ui->pushButtonMintzHinka->setToolTip(tr("zHinka is currently disabled due to maintenance."));

        ui->pushButtonSpendzHinka->setEnabled(false);
        ui->pushButtonSpendzHinka->setToolTip(tr("zHinka is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHinkapayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHinka_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHinka is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHinka...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHinka in ") +
...
void PrivacyDialog::on_pushButtonSpendzHinka_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHinka is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHinka
        sendzHinka();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHinka
    sendzHinka();
}

void PrivacyDialog::on_pushButtonZHinkaControl_clicked()
{
    ZHinkaControlDialog* zHinkaControl = new ZHinkaControlDialog(this);
    zHinkaControl->setModel(walletModel);
    zHinkaControl->exec();
}

void PrivacyDialog::setZHinkaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHinkaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHinka()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Hinka Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHinkapayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHinka is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " Hinka </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHinka</b>";
    ...
    // use mints from zHinka selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHinkaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZHinkaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zhinka selector in case it was used
    ZHinkaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHinka Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Hinka, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zHinka Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHinka </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHinka "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHinka "));
    ui->labelzHinkaAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHinkaSyncStatus->setVisible(fShow);
}
