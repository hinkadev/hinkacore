...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    //LogPrintf("XX69----------> IsBlockValueValid(): height %d, nMinted: %d, nExpectedValue: %d\n", nHeight, FormatMoney(nMinted), FormatMoney(nExpectedValue));
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            CAmount reductionFee = masternodePayment; //we need to reduce amount staking payee, but we have to take into account stake split
            ...
            if(txNew.vout[i - 1].nValue < reductionFee) {
				...
				assert(i < 2); //should never happen
